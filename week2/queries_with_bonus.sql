...
where discontinued = true order by product_id;
...
select ship_address, count(*) from orders AS order_count 
...
from us_states s left join customers c 
...
from territories t join regions r
...
select distinct c.company_name, sum(od.quantity)
